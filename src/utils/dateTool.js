...
/**
 * 计算时间差距，毫秒
 * @param {number} date1 数字
 * @param {number} date2 数字
 * @return {number} sting
 */
const timeDist = function (date1, date2) {
...
const dateTool = {
    ...
    getMaxDate: function (year, month) {
        // 处理月份天数少的情况
        return (~[4, 6, 9, 11].indexOf(month) && 30)
            // 处理二月份的闰年情况
            || (month === 2 && (dateTool.isLeapYear(year) ? 28 : 29))
            // 处理月份比较长的情况
            || 31;
    }
};
